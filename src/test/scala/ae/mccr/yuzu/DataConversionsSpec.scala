...
class DataConversionsSpec extends ScalatraSpec {
  ...
  def json = {
    val result = DataConversions.toJson(testData, testContext)
    result should contain ("\"foo\": \"bar\"")
  }

  def rdf = {
    val result = DataConversions.toRDFXML(testData, testContext)
  ...
  def turtle = {
    val result = DataConversions.toTurtle(testData, testContext)
    val model = ModelFactory.createDefaultModel()
